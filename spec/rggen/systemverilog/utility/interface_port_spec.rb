  ...
  describe InterfacePort do
    ...
    describe '#declaration' do
      it 'インターフェースポートの宣言を返す' do
        ...
        expect(interface_port(:foo_if, :foo) { array_size [2, 3] }).to match_declaration('foo_if foo[2][3]')
        expect(interface_port(:foo_if, :foo) { modport :slave, [:bar, :baz]; array_size [2, 3] }).to match_declaration('foo_if.slave foo[2][3]')
      end
    end
    ...
    describe '#identifier' do
      it '識別子を返す' do
        ...
        expect(interface_port(:foo_if, :foo) { modport :slave }).to match_identifier('foo')
        expect(interface_port(:foo_if, :foo) { array_size [2, 3] }).to match_identifier('foo')
        expect(interface_port(:foo_if, :foo) { array_size [2, 3] }.identifier[[1, 2]]).to match_identifier('foo[1][2]')
      end
      ...
      context '#modportで下位ポートが設定された場合' do
        specify '下位ポートの識別子を取得できる' do
          ...
          identifier = interface_port(:foo_if, :foo) { modport :slave, [:bar, :baz]; array_size [2, 3] }.identifier
