...
RSpec.describe RgGen::SystemVerilog::Common::Utility::DataObject do
  def data_object(name, &block)
    described_class.new([:variable, :argument, :parameter].sample, name: name, &block)
  end

  def variable(name, &block)
    described_class.new(:variable, name: name, &block)
  end

  def argument(name, direction, &block)
    described_class.new(:argument, name: name, direction: direction, &block)
  end

  def parameter(name, parameter_type, &block)
    described_class.new(:parameter, name: name, parameter_type: parameter_type, &block)
  end
